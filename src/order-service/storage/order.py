...
class OrderStorage:
    def __init__(self):
        ...
        self.__create()
        logging.debug("[ORDER-STORAGE] Storage initialized")

    def __create(self):
        if self.conn is None:
            logging.error(f"[ORDER-STORAGE] Connection error!")
    ...
    def __get_order_items(self, order: OrderDomain) -> OrderDomain:
        ...
        for item in data:
            order.add_item(OrderItemDomain(item_id=item[0], product_id=item[1], price=item[2], quantity=item[3]))
